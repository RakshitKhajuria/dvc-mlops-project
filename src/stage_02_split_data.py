...
def split_and_save(config_path, params_path):
    ...
    df = pd.read_csv(raw_local_file_path) #reading the data from directory

    split_ratio = params["base"]["test_size"] #initializing our parameters
    random_state = params["base"]["random_state"]

    train, test = train_test_split(df, test_size=split_ratio, random_state=random_state) #splitting the data

    split_data_dir = config["artifacts"]["split_data_dir"] #initializing and creating the dir
